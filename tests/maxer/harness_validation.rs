...
fn rejects_missing_value() {
    ...
    assert_eq!(
        err.to_string(),
        "max harness `fixtures/maxer/harness-validation/HarnessWithoutValue.sol:HarnessWithoutValue` must define a `value` function"
    );
}
...
fn rejects_non_view_value() {
    ...
    assert_eq!(
        err.to_string(),
        "max harness `fixtures/maxer/harness-validation/HarnessWithNonViewValue.sol:HarnessWithNonViewValue` function `value` must be `view` or `pure`, found `nonpayable`"
    );
}
...
fn rejects_wrong_value_return() {
    ...
    assert_eq!(
        err.to_string(),
        "max harness `fixtures/maxer/harness-validation/HarnessWithWrongValueReturn.sol:HarnessWithWrongValueReturn` function `value` must return `uint256`, found `uint128`"
    );
}
...
fn rejects_invariant_functions() {
    ...
    assert_eq!(
        err.to_string(),
        "max harness `fixtures/maxer/harness-validation/HarnessWithInvariantFunction.sol:HarnessWithInvariantFunction` must not define `invariant_*` functions, found: invariant_value_is_zero"
    );
}
