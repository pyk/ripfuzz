//! A failed `setup()` call after a successful deployment stops the campaign
//! like `--stop-on-revert`: the full trace is written to `fulltrace.log`, a
//! compact trace (call context and storage changes omitted) goes into the
//! campaign log and stderr, and the final error names the log and trace file
//! paths.
...
fn args(corpus_dir: impl AsRef<Path>) -> Args {
    Args {
        harness: "test/SetupRevert.sol:SetupRevert".to_owned(),
...
/// A failed `setup()` must stop the campaign after deployment, write the full
/// trace to `fulltrace.log`, dump the compact trace into the log, and name
/// both file paths in the final error.
#[test]
fn setup_failure_dumps_trace_into_log() {
    let tmp = tempfile::tempdir().unwrap();

    let before = campaign_dirs(PROJECT);
    let err =
        run(args(tmp.path().join("corpus"))).expect_err("campaign must fail when setup reverts");
    let err_str = err.to_string();
    assert!(
        err_str.contains("setup failed"),
        "error must name the failed setup: {err_str}"
    );
    ...
    assert!(
        log.contains("Setup failed."),
        "campaign log must report the failed setup:\n{log}"
    );
    assert!(
        log.contains("[revert]"),
    ...
    assert!(
        log.contains("setup always reverts"),
