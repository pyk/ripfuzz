...
contract CheatcodeLabel {
    ...
    function setup() external {
        rvm.label(TARGET, "TargetFromSetup");
    }

    function setup_label_persists() external view returns (bool) {
        return keccak256(bytes(rvm.getLabel(TARGET))) == keccak256(bytes("TargetFromSetup"));
    }

    function setup_only() external view returns (bool) {
        return bytes(rvm.getLabel(OTHER)).length == 0;
    }
    ...
    function call_getLabel(address addr) external view returns (string memory) {
        return rvm.getLabel(addr);
    }

    function label_persists_across_calls() external view returns (bool) {
        return keccak256(bytes(rvm.getLabel(OTHER))) == keccak256(bytes("OtherLabel"));
    }

    // --- Overwrite ---

    function call_label_twice() external {
        rvm.label(TARGET, "First");
        rvm.label(TARGET, "Second");
    }

    function overwrite() external view returns (bool) {
        return keccak256(bytes(rvm.getLabel(TARGET))) == keccak256(bytes("Second"));
    }

    // --- Revert safety ---

    function call_label_then_revert() external {
        rvm.label(TARGET, "RevertedLabel");
        revert("intentional");
    }

    function revert_does_not_undo_label() external view returns (bool) {
        // label is metadata, not state; it survives the revert
        return keccak256(bytes(rvm.getLabel(TARGET))) == keccak256(bytes("RevertedLabel"));
    }
    ...
    function empty_label() external view returns (bool) {
        return bytes(rvm.getLabel(TARGET)).length == 0;
    }

    // --- Edge: address(0) ---

    function call_label_zero() external {
        rvm.label(address(0), "ZeroAddress");
    }

    function label_zero() external view returns (bool) {
        return keccak256(bytes(rvm.getLabel(address(0)))) == keccak256(bytes("ZeroAddress"));
    }

    // --- setup + sequence label interaction ---

    function call_label_overrides_setup() external {
        rvm.label(TARGET, "Overridden");
    }

    function setup_override() external view returns (bool) {
        return keccak256(bytes(rvm.getLabel(TARGET))) == keccak256(bytes("Overridden"));
    }
}
