# HandlerContractBasic Coverage: Optimizer-Enabled vs Optimizer-Disabled

This document investigates why coverage reports differ between
optimizer-disabled and optimizer-enabled builds of `HandlerContractBasic.sol`,
...
`optimizer_enabled_handler_contract_basic_call_once` (in
`src/evm/coverage/reporter.rs`) deploys `HandlerContractBasic` from the
...
```solidity
contract HandlerContractBasic is RaptorFuzz {
...
but it could close the gap for simple cases like `HandlerContractBasic`.
