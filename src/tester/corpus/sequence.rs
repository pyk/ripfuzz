...
use crate::tester::corpus::call::Call;
...
impl Sequence {
    ...
    /// seeded with the extracted literals.
    pub fn random(
        rng: &mut Rng,
        handlers: &[Function],
        max_calls: usize,
        ...
        for _ in 0..len {
            ...
            calls.push(Call::random(rng, function, literals)?);
        }
        Ok(Self(calls))
    }
...
mod tests {
    ...
    fn random_length_within_bounds() {
        let mut rng = Rng::new();
        for _ in 0..32 {
            let sequence = Sequence::random(&mut rng, &handlers(), 3, &literals()).unwrap();
    ...
    fn random_calls_use_handlers() {
        let mut rng = Rng::new();
        let sequence = Sequence::random(&mut rng, &handlers(), 4, &literals()).unwrap();
    ...
    fn random_with_empty_handlers_fails() {
        let mut rng = Rng::new();
        let err = Sequence::random(&mut rng, &[], 4, &literals()).unwrap_err();
    ...
    fn display_joins_signatures() {
        let mut rng = Rng::new();
        let sequence = Sequence::random(&mut rng, &handlers(), 1, &literals()).unwrap();
