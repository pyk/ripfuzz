...
impl Drop for DedupGuard<'_> {
    fn drop(&mut self) {
        if self.active {
            ...
            let e = anyhow!("dedup caller panicked");
            self.table.complete(self.key, Err(&e));
        }
    }
}
...
impl DedupTable {
    ...
    pub fn complete(&self, key: &str, result: Result<&serde_json::Value, &anyhow::Error>) {
...
mod tests {
    ...
    fn dedup_error_propagation() {
        ...
        std::thread::sleep(std::time::Duration::from_millis(10));
        let e = anyhow::anyhow!("network down");
        table.complete(key, Err(&e));
