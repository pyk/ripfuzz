...
mod tests {
    ...
    fn get_label_returns_empty_for_unknown() {
        ...
        let mut ctx = revm::context::Context::mainnet().with_db(InMemoryDB::default());
        let outcome =
            crate::evm::cheatcode::build_outcome(&effects, 1_000_000, &mut ctx, &inspector.state);
    ...
    fn label_setup_integration() {
        let artifact =
            contract::tests::load_test_artifact("fixtures/cheatcodes", "test/CheatcodeLabel.sol")
                .unwrap();
