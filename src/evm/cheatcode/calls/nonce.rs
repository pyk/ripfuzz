...
mod tests {
    ...
    fn deploy_and_setup() -> (Chain, Address) {
        let contract = load_fixture("src/NonceHarness.sol:NonceHarness");
    ...
    fn nonce_set_in_setup_matches_expected() {
        ...
        let txs = vec![Transaction::new(target).calldata(Bytes::from(
            NonceHarness::invariant_nonceAtLeastBaselineCall::new(()).abi_encode(),
        ))];
    ...
    fn nonce_persists_from_setup_into_exec() {
        let (mut chain, target) = deploy_and_setup();
        let txs = vec![Transaction::new(target).calldata(Bytes::from(
            NonceHarness::getNonceDirectCall::new(()).abi_encode(),
        ))];

        let execution = chain.exec(&txs).unwrap();
        assert_eq!(execution.results.len(), 1);
        assert!(execution.results[0].success, "getNonceDirect must succeed");
        let ret = NonceHarness::getNonceDirectCall::abi_decode_returns(
    ...
    fn bump_nonce_in_action_increases_value() {
        ...
        let txs = vec![
            Transaction::new(target).calldata(Bytes::from(
                NonceHarness::actionBumpNonceCall::new(()).abi_encode(),
            )),
            Transaction::new(target).calldata(Bytes::from(
                NonceHarness::getStoredNonceCall::new(()).abi_encode(),
            )),
            Transaction::new(target).calldata(Bytes::from(
                NonceHarness::invariant_nonceAtLeastBaselineCall::new(()).abi_encode(),
            )),
        ...
        let stored: U256 = NonceHarness::getStoredNonceCall::abi_decode_returns(
    ...
    fn bump_nonce_sequence_accumulates() {
        ...
        let txs = vec![
            ...
            Transaction::new(target).calldata(Bytes::from(
                NonceHarness::actionBumpNonceByTwoCall::new(()).abi_encode(),
            )),
            Transaction::new(target).calldata(Bytes::from(
                NonceHarness::getStoredNonceCall::new(()).abi_encode(),
            )),
    ...
    fn overwrite_sequence_in_single_transaction() {
        ...
        let txs = vec![
            Transaction::new(target).calldata(Bytes::from(
                NonceHarness::actionOverwriteSequenceCall::new(()).abi_encode(),
            )),
            Transaction::new(target).calldata(Bytes::from(
                NonceHarness::getStoredNonceCall::new(()).abi_encode(),
            )),
    ...
    fn revert_low_nonce_in_action_reverts() {
        let (mut chain, target) = deploy_and_setup();
        let txs = vec![Transaction::new(target).calldata(Bytes::from(
            NonceHarness::actionRevertLowNonceCall::new(()).abi_encode(),
        ))];
